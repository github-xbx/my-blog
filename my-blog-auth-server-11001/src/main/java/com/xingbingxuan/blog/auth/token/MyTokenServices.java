package com.xingbingxuan.blog.auth.token;
...
/**
 * 重写DefaultTokenServices 实现token每次都刷新
 * @Author xbx
 * @Date 10:24 2022/8/31
 **/
public class MyTokenServices implements AuthorizationServerTokenServices, ResourceServerTokenServices,
    ...
    public OAuth2AccessToken createAccessToken(OAuth2Authentication authentication) throws AuthenticationException {
        ...
        if (existingAccessToken != null) {
            if (existingAccessToken.getRefreshToken() != null) {
                ...
                //在数据库中删除这个refresh_token 信息
                tokenStore.removeRefreshToken(refreshToken);
            }
            //在数据库中删除这个access_token 信息
            tokenStore.removeAccessToken(existingAccessToken);

        }

        //创建 refresh_token 每个access_token 都对应一个新的 refresh_token
        refreshToken = createRefreshToken(authentication);
        //创建 新的access_token
        OAuth2AccessToken accessToken = createAccessToken(authentication, refreshToken);
        //存入数据库 access_token 信息
        tokenStore.storeAccessToken(accessToken, authentication);

        refreshToken = accessToken.getRefreshToken();
        if (refreshToken != null) {
            //存入数据库 refresh_token 信息
    ...
    //TODO 更改refresh token返回的数据类型 “xxxx-xxxx-xxxx” -> “xxxxxxxxxxxx”
    ...
    //TODO 更改token返回的数据类型 “xxxx-xxxx-xxxx” -> “xxxxxxxxxxxx”
    private OAuth2AccessToken createAccessToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {

        DefaultOAuth2AccessToken token = new DefaultOAuth2AccessToken(UUID.randomUUID().toString().replaceAll("-", ""));
